...
function grcFallbackDirectory(): string {
  if (process.env.VERCEL || process.env.AWS_LAMBDA_FUNCTION_NAME) {
    return path.join("/tmp", "project-vail-grc-fallback");
  }
...
export async function persistVibeDecision(input: VibeTransactionInput): Promise<PersistVibeDecisionResult> {
  ...
  const row = {
    traceId,
    tenantId: asUuidOrNull(meta.tenant_id ?? meta.tenantId),
  ...
  try {
    ...
    console.error(
      `[project-vail-mcp] log_vibe_transaction: Postgres persist failed (${err}). Wrote fallback: ${fp}`,
    );
...
export async function generateAuditReportMarkdown(params: {
  ...
  try {
    ...
    lines.push("");
    lines.push("_Generated by Project Vail MCP `generate_audit_report`. Not a legal attestation._");
