...
import { utilizationBarFill } from "../lib/utils";
...
const utilizationBarCaption =
  "Bar color matches capacity bands: amber under 70%, green 70–90%, red over 90%.";
    ...
    <section className="grid gap-4 lg:grid-cols-2">
      <Card className="border-slate-700 bg-slate-950/40">
        <CardHeader className="pb-2">
          ...
          <p className="text-xs text-muted-foreground">{utilizationBarCaption}</p>
        </CardHeader>
        <CardContent className="h-80">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={byGeo}>
              ...
              <Bar dataKey="averageUtilizationPct" name="Utilization %" radius={[4, 4, 0, 0]}>
                {byGeo.map((entry, index) => (
                  <Cell key={`${entry.label}-${index}`} fill={utilizationBarFill(entry.averageUtilizationPct)} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
